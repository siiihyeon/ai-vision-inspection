...
성공은 frame 수신이 아니라 RGB PNG atomic 저장·digest/readability 확인과 `InferenceJob` enqueue까지 끝난 시점입니다. Queue full 동안 Action은 `ENQUEUE_BLOCKED` feedback을 반복하며 종료하지 않습니다. 취소 또는 복구 실패만 terminal failure입니다. 정상 결과는 `error_code=0`, `reason="OK"`입니다.
