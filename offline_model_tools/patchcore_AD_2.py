...
def _save_normalized_heatmap(
    ...
    *,
    ...
    _save_rgb(path, np.concatenate((banner, overlay), axis=0))
...
def evaluate(
    *,
    ...
    for split, label, label_name in split_specs:
        ...
        for number, filename in enumerate(names, start=1):
            ...
            for station_position, current_views in enumerate(station_views):
                ...
                station_pipeline[station_name].append(station_results[-1]["pipeline_ms"])
            ...
            if product_prediction != bool(label):
                ...
                for current_views, station_result in zip(station_views, station_results, strict=True):
                    for position, view in enumerate(current_views):
                        ...
                        _save_rgb(product_dir / f"{view}_model_input.png", _tensor_rgb(prepared.tensor))
    ...
    timing = {
        "definition": {
            "model_pipeline": "GPU input ready through native map, spatial normalization, aggregation and verdict",
        ...
        "station_a": {
            "model_pipeline": _timing_summary(station_pipeline["station_a"]),
            "end_to_end": _timing_summary(station_end_to_end["station_a"]),
        },
        "station_b": {
            "model_pipeline": _timing_summary(station_pipeline["station_b"]),
